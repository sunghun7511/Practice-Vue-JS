...
export const INCREMENT_TIMER = 'INCREMENT_TIME';
    ...
    mutations: {
        ...
        [OPEN_CELL](state, {row, cell}) {
            ...
            function checkAround(row, cell) {
                const check = row < 0 || row >= state.tableData.length || cell < 0 || cell >= state.tableData[0].length;
                if (check) {
                    return;
                }
                ...
                Vue.set(state.tableData[row], cell, count === 0 ? '' : count.toString());
            }
            ...
            if (state.data.row * state.data.cell - state.data.mine === state.openedCount) {
                state.halted = true;
                state.result = `${state.timer}초만에 성공하셨습니다.`;
            }
        },
